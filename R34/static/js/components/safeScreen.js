...
export class SafeScreen {
    ...
    async renderFileListContainer(containerEl) {
        ...
        containerEl.innerHTML = '<div class="safe-screen-empty">Загрузка списка файлов...</div>';

        const files = await this.getFiles();
        if (files.length === 0) {
            containerEl.innerHTML = '<div class="safe-screen-empty">Папка safe_screen пуста</div>';
            return;
        }
        ...
        files.forEach(file => {
            ...
            itemEl.className = 'safe-screen-file-item';

            const infoEl = document.createElement('div');
            infoEl.className = 'safe-screen-file-info';

            const badge = document.createElement('span');
            badge.className = 'safe-screen-badge';
            badge.textContent = file.type === 'video' ? 'видео' : 'фото';

            const nameEl = document.createElement('span');
            nameEl.className = 'safe-screen-filename';
            ...
            delBtn.className = 'safe-screen-del-btn';
            ...
            delBtn.onclick = async (e) => {
                ...
                if (ok) {
                    ...
                    delBtn.classList.add('error');
                    delBtn.textContent = 'Ошибка!';
                    setTimeout(() => {
                        delBtn.classList.remove('error');
                        resetBtn();
                    }, 2000);
                }
            };
